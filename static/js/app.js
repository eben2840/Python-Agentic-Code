...
// ============================================================================
// GENERATE IDEA FUNCTIONALITY
// ============================================================================

async function generateIdea() {
  const promptInput = document.getElementById('idea-prompt');
  const resultSection = document.getElementById('idea-result-section');
  const loadingSection = document.getElementById('idea-loading');
  const ideaContent = document.getElementById('idea-content');
  const generateBtn = document.getElementById('generate-idea-btn');

  const prompt = promptInput?.value?.trim();
  if (!prompt) {
    showToast('Please enter a description for your SMART on FHIR application', 'warning');
    return;
  }
  ...
  try {
    ...
    // Store the idea for later use
    window.app.currentIdea = {
      prompt: prompt,
      content: data.idea || data.content,
      title: extractTitleFromIdea(data.idea || data.content)
    };

    // Display the generated idea
    if (ideaContent) {
      ideaContent.innerHTML = formatIdeaContent(data.idea || data.content);
    }
    ...
    showToast(`Failed to generate idea: ${error.message}`, 'error');
  } finally {
...
function extractTitleFromIdea(content) {
  ...
  return 'SMART on FHIR Mini App';
}

function formatIdeaContent(content) {
  let html = content
...
// Generate Idea button handler
document.getElementById('generate-idea-btn')?.addEventListener('click', generateIdea);
document.getElementById('regenerate-idea-btn')?.addEventListener('click', generateIdea);
...
async function createTask(event) {
  ...
  const specification = document.getElementById('task-specification')?.value?.trim();
  const complexity = document.getElementById('task-complexity')?.value || 'standard';
  ...
  try {
    ...
    document.getElementById('task-specification').value = '';

    // Switch to dashboard view
    switchToView('dashboard');

    // Add the new task card to the pending column then auto-start it
    addTaskCardToColumn(data.task_id, title, description, 'pending');

    // Auto-start the task
    window.app.currentTaskId = data.task_id;
    const runResponse = await fetch(`/api/tasks/${data.task_id}/run`, { method: 'POST' });
    const runData = await runResponse.json();

    if (!runData.error) {
      moveTaskCard(data.task_id, 'planning');
      startTaskPolling(data.task_id);
      showToast('Task created and started!', 'success');
    } else {
      showToast('Task created! Click on it to start.', 'success');
    }
...
function addTaskCardToColumn(taskId, title, description, columnStatus) {
  const columnBody = document.querySelector(`[data-status="${columnStatus}"] [data-droppable]`);
  if (!columnBody) return;

  // Remove empty state if present
  const emptyState = columnBody.querySelector('.empty-state-text');
  ...
  card.innerHTML = `
    <div class="task-card-header">
      ...
      <span class="task-complexity complexity-standard">standard</span>
    </div>
    <h4 class="task-card-title">${title}</h4>
    ${description ? `<p class="task-card-description">${description.substring(0, 100)}${description.length > 100 ? '...' : ''}</p>` : ''}
    <div class="task-card-footer">
      <span class="task-status-label">Click to Start</span>
...
document.getElementById('submit-task-btn')?.addEventListener('click', createTask);
